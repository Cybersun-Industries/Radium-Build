...
using Content.Server.Radium.Vampire.Components;
...
public sealed class VampireRuleSystem : GameRuleSystem<VampireRuleComponent>
{
    ...
    protected override void ActiveTick(EntityUid uid, VampireRuleComponent component, GameRuleComponent gameRule, float frameTime)
    {
        ...
        if (component.SelectionStatus == VampireRuleComponent.SelectionState.ReadyToSelect && _gameTiming.CurTime > component.AnnounceAt)
            DoTraitorStart(component);
    }

    private void OnStartAttempt(RoundStartAttemptEvent ev)
    {
        var query = EntityQueryEnumerator<VampireRuleComponent, GameRuleComponent>();
    ...
    private void MakeCodewords(VampireRuleComponent component)
    {
    ...
    private void DoTraitorStart(VampireRuleComponent component)
    {
        ...
        component.SelectionStatus = VampireRuleComponent.SelectionState.SelectionMade;
    }
    ...
    private void OnPlayersSpawned(RulePlayerJobsAssignedEvent ev)
    {
        ...
        while (query.MoveNext(out var uid, out var traitor, out var gameRule))
        {
            ...
            traitor.SelectionStatus = VampireRuleComponent.SelectionState.ReadyToSelect;
        }
    }

    private List<ICommonSession> FindPotentialTraitors(in Dictionary<ICommonSession, HumanoidCharacterProfile> candidates, VampireRuleComponent component)
    {
    ...
    public bool MakeVampire(ICommonSession traitor, bool giveUplink = true, bool giveObjectives = true)
    {
        var traitorRule = EntityQuery<VampireRuleComponent>().FirstOrDefault();
        if (traitorRule == null)
        {
            //todo fuck me this shit is awful
            //no i wont fuck you, erp is against rules
            GameTicker.StartGameRule("Traitor", out var ruleEntity);
            traitorRule = Comp<VampireRuleComponent>(ruleEntity);
        ...
        if (HasComp<VampireRoleComponent>(mindId))
        {
    ...
    private void HandleLatejoin(PlayerSpawnCompleteEvent ev)
    {
        ...
        while (query.MoveNext(out var uid, out var traitor, out var gameRule))
        {
            ...
            if (traitor.SelectionStatus < VampireRuleComponent.SelectionState.SelectionMade)
            {
    ...
    public List<(EntityUid Id, MindComponent Mind)> GetOtherTraitorMindsAliveAndConnected(MindComponent ourMind)
    {
        ...
        foreach (var traitor in EntityQuery<VampireRuleComponent>())
        {
    ...
    private List<(EntityUid Id, MindComponent Mind)> GetOtherTraitorMindsAliveAndConnected(MindComponent ourMind, VampireRuleComponent component)
    {
