changeling-chemicals-deficit = Недостаточно ресурсов!
#UI
changeling-user-interface-title = Окно эволюции
changeling-user-interface-cost = { $price } ресурсов
changeling-condition-genes-description = Используйте своё жало, чтобы извелечь гены нескольких человек.
changeling-defibrillator-grasp-used = Генокрад оживает и отрывает руки ближайшему человеку!

changeling-transform-window-title = Хранилище ДНК
changeling-absorbed-corpse-description = Личность невозможно определить. Кожа имеет пепельно-серый цвет.
changeling-absorbed-corpse-detailed-description = Похоже, что сущность имеет обширные ожоги внутренних органов. Вы явно не хотите занть, что с ним случилось.
changeling-absorbed-corpse-name = Неопознанное тело

changeling-action-fail-lesserform = Невозможно использовать в форме обезьяны!

changeling-absorb-start = Генокрад начал поглощать тело!
changeling-absorb-fail-incapacitated =  Цель должна быть оглушена или связана!
changeling-absorb-fail-absorbed = Вы уже поглотили это.
changeling-absorb-end-self = Ещё одна цель поглощена. Вы эволюционируете.

changeling-sting = Вы незаметно жалите {CAPITALIZE(THE($target))}
changeling-sting-fail-simplemob = Вы не можете ужалить примитивное существо!

changeling-stasis-enter = Вы входите в стазис.
changeling-stasis-exit-fail = Вы выходите из стазиса.

changeling-hand-transform-end = Ваши руки приобретают первоначальный вид.
changeling-fail-hands = Сперва нужно освободить руки.

changeling-armblade-start = Ваша рука превращается в оргомное лезвие.
changeling-shield-start = Ваша рука меняет форму и превращается в щит.

changeling-muscles-start = Ваше тело ощущается более лёгким.
changeling-muscles-end = Ваши ноги тяжелеют.

changeling-equip-armor-fail = Сперва нужно снять текущую одежду.
changeling-equip-armor-start = Ваше тело покрывает крепкая хитиновая оболочка.
changeling-equip-spacesuit-start = Ваше тело покрывается герметичной оболочкой.
changeling-equip-end = Ваше тело приобретает первоначальный вид.

changeling-fleshmend = Ваше тело сжимается, заживляя раны и восстанавливая мертвые клетки.
changeling-panacea = Вы мутируете и изменяете свою ДНК для лучшей регенерации клеток.

changeling-chameleon-start = Вы адаптируете свою кожу к окружающей среде.
changeling-chameleon-end = Ваша кожа теряет прозрачность.
